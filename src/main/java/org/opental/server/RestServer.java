...
import org.opental.core.logging.RequestLoggingFilter;
...
public class RestServer {
	...
	public boolean startServer() {
		...
		while (plugins.hasNext()) {
			final TestAdaptationPlugin robo = plugins.next();
			// register old-style adapter
			if (robo.getType() == TestAdaptationType.ADAPTER && robo.getImplementation() != null) {
				logger.info("register old style: {}", robo.getName());
				config.register(robo.getImplementation());
			} else if (robo.getType() == TestAdaptationType.ADAPTER && robo.getImplementation() == null) {
